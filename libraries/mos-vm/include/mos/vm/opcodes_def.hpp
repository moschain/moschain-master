...
/* clang-format off */
#define MOS_VM_CONTROL_FLOW_OPS(opcode_macro)   \
...
#define MOS_VM_BR_TABLE_OP(opcode_macro)        \
   opcode_macro(br_table, 0x0E)
#define MOS_VM_RETURN_OP(opcode_macro)          \
   opcode_macro(return_, 0x0F)
#define MOS_VM_CALL_OPS(opcode_macro)           \
   opcode_macro(call, 0x10)                     \
   opcode_macro(call_indirect, 0x11)            \
   opcode_macro(br_table_data, 0x12)
#define MOS_VM_CALL_IMM_OPS(opcode_macro)       \
...
#define MOS_VM_PARAMETRIC_OPS(opcode_macro)     \
...
#define MOS_VM_VARIABLE_ACCESS_OPS(opcode_macro)\
...
#define MOS_VM_MEMORY_OPS(opcode_macro)         \
...
#define MOS_VM_I32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i32_const, 0x41)
#define MOS_VM_I64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i64_const, 0x42)
#define MOS_VM_F32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f32_const, 0x43)
#define MOS_VM_F64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f64_const, 0x44)
#define MOS_VM_COMPARISON_OPS(opcode_macro)     \
...
#define MOS_VM_NUMERIC_OPS(opcode_macro)        \
...
#define MOS_VM_CONVERSION_OPS(opcode_macro)     \
...
#define MOS_VM_EXIT_OP(opcode_macro)            \
   opcode_macro(exit, 0xC0)
#define MOS_VM_EMPTY_OPS(opcode_macro)          \
...
#define MOS_VM_ERROR_OPS(opcode_macro)          \
   opcode_macro(error, 0xFF)

/* clang-format on */

#define MOS_VM_CREATE_ENUM(name, code) name = code,

#define MOS_VM_CREATE_STRINGS(name, code) #name,

#define MOS_VM_CREATE_MAP(name, code) { code, #name },

#define MOS_VM_OPCODE_NAME_if_
#define MOS_VM_OPCODE_NAME_else_
#define MOS_VM_OPCODE_NAME_return_

#define MOS_VM_OPCODE_NAME_TEST() 1
#define MOS_VM_OPCODE_NAME_TEST_MOS_VM_OPCODE_NAME_TEST 0,
#define MOS_VM_OPCODE_NAME_TEST_1 1, ignore
#define MOS_VM_EXPAND(x) x
#define MOS_VM_CAT2(x, y) x ## y
#define MOS_VM_CAT(x, y) MOS_VM_CAT2(x, y)
#define MOS_VM_APPLY(f, args) f args
#define MOS_VM_FIX_OPCODE_NAME_0(name) name ## _t
#define MOS_VM_FIX_OPCODE_NAME_1(name) name ## t
#define MOS_VM_FIX_OPCODE_NAME(iskeyword, garbage) MOS_VM_FIX_OPCODE_NAME_ ## iskeyword

#define MOS_VM_OPCODE_T(name)                                                             \
   MOS_VM_APPLY(MOS_VM_FIX_OPCODE_NAME,                                                   \
      (MOS_VM_CAT(MOS_VM_OPCODE_NAME_TEST_,                                               \
           MOS_VM_EXPAND(MOS_VM_OPCODE_NAME_TEST MOS_VM_OPCODE_NAME_ ## name ()))))(name)

#define MOS_VM_CREATE_EXIT_TYPE(name, code)                                                                      \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t pc;                                                                                                     \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define MOS_VM_CREATE_CONTROL_FLOW_TYPES(name, code)                                                                   \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() {}                                                                                       \
      MOS_VM_OPCODE_T(name)(uint32_t data) : data(data) {}                                                             \
      MOS_VM_OPCODE_T(name)(uint32_t d, uint32_t pc, uint16_t i, uint16_t oi)                                          \
...
#define MOS_VM_CREATE_BR_TABLE_TYPE(name, code)                                                                        \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
...
#define MOS_VM_CREATE_TYPES(name, code)                                                                                \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define MOS_VM_CREATE_CALL_TYPES(name, code)                                                                           \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define MOS_VM_CREATE_CALL_IMM_TYPES(name, code)                                                                       \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      uint16_t locals;                                                                                                 \
...
#define MOS_VM_CREATE_VARIABLE_ACCESS_TYPES(name, code)                                                                \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
...
#define MOS_VM_CREATE_MEMORY_TYPES(name, code)                                                                         \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
...
#define MOS_VM_CREATE_I32_CONSTANT_TYPE(name, code)                                                                    \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
      explicit MOS_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit MOS_VM_OPCODE_T(name)(int32_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         int32_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define MOS_VM_CREATE_I64_CONSTANT_TYPE(name, code)                                                                    \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
      explicit MOS_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit MOS_VM_OPCODE_T(name)(int64_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         int64_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define MOS_VM_CREATE_F32_CONSTANT_TYPE(name, code)                                                                    \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
      explicit MOS_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit MOS_VM_OPCODE_T(name)(float n) { data.f = n; }                                                          \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         float    f;                                                                                                   \
      } data;                                                                                                          \
...
#define MOS_VM_CREATE_F64_CONSTANT_TYPE(name, code)                                                                    \
   struct MOS_VM_OPCODE_T(name) {                                                                                      \
      MOS_VM_OPCODE_T(name)() = default;                                                                               \
      explicit MOS_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit MOS_VM_OPCODE_T(name)(double n) { data.f = n; }                                                         \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         double   f;                                                                                                   \
      } data;                                                                                                          \
...
#define MOS_VM_IDENTITY(name, code) mos::vm::MOS_VM_OPCODE_T(name),
#define MOS_VM_IDENTITY_END(name, code) mos::vm::MOS_VM_OPCODE_T(name)
