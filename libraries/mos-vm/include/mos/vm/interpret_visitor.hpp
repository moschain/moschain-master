...
#include <mos/vm/config.hpp>
#include <mos/vm/base_visitor.hpp>
#include <mos/vm/exceptions.hpp>
#include <mos/vm/opcodes.hpp>
#include <mos/vm/softfloat.hpp>
#include <mos/vm/stack_elem.hpp>
#include <mos/vm/utils.hpp>
#include <mos/vm/wasm_stack.hpp>
...
namespace mos { namespace vm {
   ...
   struct interpret_visitor : base_visitor {
      ...
      [[gnu::always_inline]] inline void operator()(const call_indirect_t& op) {
         ...
         MOS_VM_ASSERT(actual_type == expected_type, wasm_interpreter_exception, "bad call_indirect type");
         context.call(fn);
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i32_div_s_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_i32();
         auto&       lhs = context.peek_operand().to_i32();
         MOS_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i32.div_s divide by zero");
         MOS_VM_ASSERT(!(lhs == std::numeric_limits<int32_t>::min() && rhs == -1), wasm_interpreter_exception,
      ...
      [[gnu::always_inline]] inline void operator()(const i32_div_u_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_ui32();
         auto&       lhs = context.peek_operand().to_ui32();
         MOS_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i32.div_u divide by zero");
         lhs /= rhs;
      }
      [[gnu::always_inline]] inline void operator()(const i32_rem_s_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_i32();
         auto&       lhs = context.peek_operand().to_i32();
         MOS_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i32.rem_s divide by zero");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_rem_u_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_ui32();
         auto&       lhs = context.peek_operand().to_ui32();
         MOS_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i32.rem_u divide by zero");
         lhs %= rhs;
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i64_div_s_t& op) {
         ...
         MOS_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i64.div_s divide by zero");
         MOS_VM_ASSERT(!(lhs == std::numeric_limits<int64_t>::min() && rhs == -1), wasm_interpreter_exception,
      ...
      [[gnu::always_inline]] inline void operator()(const i64_div_u_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_ui64();
         auto&       lhs = context.peek_operand().to_ui64();
         MOS_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i64.div_u divide by zero");
         lhs /= rhs;
      }
      [[gnu::always_inline]] inline void operator()(const i64_rem_s_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_i64();
         auto&       lhs = context.peek_operand().to_i64();
         MOS_VM_ASSERT(rhs != 0, wasm_interpreter_exception, "i64.rem_s divide by zero");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_trunc_s_f32_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            MOS_VM_ASSERT(!((af >= 2147483648.0f) || (af < -2147483648.0f)), wasm_interpreter_exception, "Error, f32.trunc_s/i32 overflow" );
            MOS_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f32.trunc_s/i32 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_trunc_u_f32_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            float af = oper.to_f32();
            MOS_VM_ASSERT(!((af >= 4294967296.0f) || (af <= -1.0f)),wasm_interpreter_exception, "Error, f32.trunc_u/i32 overflow");
            MOS_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f32.trunc_u/i32 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_trunc_s_f64_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            MOS_VM_ASSERT(!((af >= 2147483648.0) || (af < -2147483648.0)), wasm_interpreter_exception, "Error, f64.trunc_s/i32 overflow");
            MOS_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f64.trunc_s/i32 unrepresentable");
            oper = i32_const_t{ static_cast<int32_t>(af) };
         }
      }
      [[gnu::always_inline]] inline void operator()(const i32_trunc_u_f64_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            double af = oper.to_f64();
            MOS_VM_ASSERT(!((af >= 4294967296.0) || (af <= -1.0)), wasm_interpreter_exception, "Error, f64.trunc_u/i32 overflow");
            MOS_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f64.trunc_u/i32 unrepresentable");
            oper = i32_const_t{ static_cast<uint32_t>(af) };
         }
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i64_trunc_s_f32_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            float af = oper.to_f32();
            MOS_VM_ASSERT(!((af >= 9223372036854775808.0f) || (af < -9223372036854775808.0f)), wasm_interpreter_exception, "Error, f32.trunc_s/i64 overflow");
            MOS_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f32.trunc_s/i64 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i64_trunc_u_f32_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            float af = oper.to_f32();
            MOS_VM_ASSERT(!((af >= 18446744073709551616.0f) || (af <= -1.0f)), wasm_interpreter_exception, "Error, f32.trunc_u/i64 overflow");
            MOS_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f32.trunc_u/i64 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i64_trunc_s_f64_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            double af = oper.to_f64();
            MOS_VM_ASSERT(!((af >= 9223372036854775808.0) || (af < -9223372036854775808.0)), wasm_interpreter_exception, "Error, f64.trunc_s/i64 overflow");
            MOS_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f64.trunc_s/i64 unrepresentable");
            oper = i64_const_t{ static_cast<int64_t>(af) };
         }
      }
      [[gnu::always_inline]] inline void operator()(const i64_trunc_u_f64_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            double af = oper.to_f64();
            MOS_VM_ASSERT(!((af >= 18446744073709551616.0) || (af <= -1.0)), wasm_interpreter_exception, "Error, f64.trunc_u/i64 overflow");
            MOS_VM_ASSERT(!__builtin_isnan(af), wasm_interpreter_exception, "Error, f64.trunc_u/i64 unrepresentable");
            oper = i64_const_t{ static_cast<uint64_t>(af) };
         }
      }
