...
#define MEMORY_DUMP_OP_VISIT(name, code) \
   void operator()(const MOS_VM_OPCODE_T(name)& op) { \
      stream << #name << "\n"; \
   }
...
#define MEMORY_DUMP_CONST_VISIT(name, code) \
   ...
   struct memory_dump_visitor {
      ...
      MOS_VM_CONTROL_FLOW_OPS(MEMORY_DUMP_CONTROL_FLOW_VISIT)
      MOS_VM_BR_TABLE_OP(MEMORY_DUMP_BR_TABLE_VISIT)
      MOS_VM_RETURN_OP(MEMORY_DUMP_OP_VISIT)
      MOS_VM_CALL_OPS(MEMORY_DUMP_CALL_VISIT)
      MOS_VM_CALL_IMM_OPS(MEMORY_DUMP_CALL_VISIT)
      MOS_VM_PARAMETRIC_OPS(MEMORY_DUMP_OP_VISIT)
      MOS_VM_VARIABLE_ACCESS_OPS(MEMORY_DUMP_VARIABLE_ACCESS_VISIT)
      MOS_VM_I32_CONSTANT_OPS(MEMORY_DUMP_CONST_VISIT)
      MOS_VM_I64_CONSTANT_OPS(MEMORY_DUMP_CONST_VISIT)
      MOS_VM_F32_CONSTANT_OPS(MEMORY_DUMP_CONST_VISIT)
      MOS_VM_F64_CONSTANT_OPS(MEMORY_DUMP_CONST_VISIT)
      MOS_VM_COMPARISON_OPS(MEMORY_DUMP_OP_VISIT)
      MOS_VM_NUMERIC_OPS(MEMORY_DUMP_OP_VISIT)
      MOS_VM_CONVERSION_OPS(MEMORY_DUMP_OP_VISIT)
      MOS_VM_EXIT_OP(MEMORY_DUMP_OP_VISIT)
      MOS_VM_EMPTY_OPS(MEMORY_DUMP_OP_VISIT)
      MOS_VM_ERROR_OPS(MEMORY_DUMP_OP_VISIT)
      template <typename T>
      ...
      public:
         ...
         void write(Stream&& stream) {
            ...
            for (; index < _size; index++) {
               mos::vm::visit(std::move(md), std::move(_opcodes[index]));
            }
         }
      private:
