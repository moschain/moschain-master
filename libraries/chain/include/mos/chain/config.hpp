...
namespace mos { namespace chain { namespace config {
...
const static name system_account_name    { N(mos) };
const static name null_account_name      { N(mos.null) };
const static name producers_account_name { N(mos.prods) };
...
const static name mos_auth_scope       { N(mos.auth) };
const static name mos_all_scope        { N(mos.all) };

const static name active_name     { N(active) };
const static name owner_name      { N(owner) };
const static name mos_any_name  { N(mos.any) };
const static name mos_code_name { N(mos.code) };
...
const static mos::chain::wasm_interface::vm_type default_wasm_runtime = mos::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace mos::chain::config

constexpr uint64_t MOS_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / mos::chain::config::percent_100;
}

template<typename Number>
Number MOS_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + mos::chain::config::percent_100 - mos::chain::config::percent_1)  / mos::chain::config::percent_100;
}
