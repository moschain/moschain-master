...
#include <mos/chain/webassembly/wavm.hpp>
#include <mos/chain/webassembly/wabt.hpp>
#ifdef MOS_VM_OC_RUNTIME_ENABLED
#include <mos/chain/webassembly/mos-vm-oc.hpp>
#else
#define _REGISTER_MOSVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
#endif
#include <mos/chain/webassembly/mos-vm.hpp>
...
using namespace fc;
using namespace mos::chain::webassembly;
using namespace IR;
   ...
   struct wasm_interface_impl {
...
#ifdef MOS_VM_OC_RUNTIME_ENABLED
      struct mosvmoc_tier {
         mosvmoc_tier(const boost::filesystem::path& d, const mosvmoc::config& c, const chainbase::database& db) : cc(d, c, db), exec(cc) {}
         mosvmoc::code_cache_async cc;
         mosvmoc::executor exec;
         mosvmoc::memory mem;
      };
#endif

      wasm_interface_impl(wasm_interface::vm_type vm, bool mosvmoc_tierup, const chainbase::database& d, const boost::filesystem::path data_dir, const mosvmoc::config& mosvmoc_config) : db(d), wasm_runtime_time(vm) {
         if(vm == wasm_interface::vm_type::wabt)
            runtime_interface = std::make_unique<webassembly::wabt_runtime::wabt_runtime>();
#ifdef MOS_MOS_VM_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::mos_vm)
            runtime_interface = std::make_unique<webassembly::mos_vm_runtime::mos_vm_runtime<mos::vm::interpreter>>();
#endif
#ifdef MOS_MOS_VM_JIT_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::mos_vm_jit)
            runtime_interface = std::make_unique<webassembly::mos_vm_runtime::mos_vm_runtime<mos::vm::jit>>();
#endif
#ifdef MOS_VM_OC_RUNTIME_ENABLED
         if(vm == wasm_interface::vm_type::mos_vm_oc)
            runtime_interface = std::make_unique<webassembly::mosvmoc::mosvmoc_runtime>(data_dir, mosvmoc_config, d);
#endif
         if(!runtime_interface)
            MOS_THROW(wasm_exception, "${r} wasm runtime not supported on this platform and/or configuration", ("r", vm));

#ifdef MOS_VM_OC_RUNTIME_ENABLED
         if(mosvmoc_tierup) {
            MOS_ASSERT(vm != wasm_interface::vm_type::mos_vm_oc, wasm_exception, "You can't use MOS VM OC as the base runtime when tier up is activated");
            mosvmoc.emplace(data_dir, mosvmoc_config, d);
         }
#endif
      }
      ...
      std::vector<uint8_t> parse_initial_memory(const Module& module) {
         ...
         for(const DataSegment& data_segment : module.dataSegments) {
            MOS_ASSERT(data_segment.baseOffset.type == InitializerExpression::Type::i32_const, wasm_exception, "");
            MOS_ASSERT(module.memories.defs.size(), wasm_exception, "");
...
#ifdef MOS_VM_OC_RUNTIME_ENABLED
         if(mosvmoc) for(auto it = first_it; it != last_it; it++)
            mosvmoc->cc.free_code(it->code_hash, it->vm_version);
#endif
         ...
         if(!it->module) {
            ...
            if (runtime_interface->inject_module(module)) {
               try {
                  ...
                  MOS_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
               } catch (const IR::ValidationException& e) {
                  MOS_ASSERT(false, wasm_serialization_error,
                             e.message.c_str());
...
#ifdef MOS_VM_OC_RUNTIME_ENABLED
      fc::optional<mosvmoc_tier> mosvmoc;
#endif
   };
...
#define _REGISTER_INTRINSIC_EXPLICIT(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)\
   ...
   _REGISTER_MOS_VM_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)       \
   _REGISTER_MOSVMOC_INTRINSIC(CLS, MOD, METHOD, WASM_SIG, NAME, SIG)
...
#define _REGISTER_INJECTED_INTRINSIC(R, CLS, INFO)\
   BOOST_PP_CAT(BOOST_PP_OVERLOAD(_REGISTER_INTRINSIC, _UNWRAP_SEQ INFO) _EXPAND_ARGS(CLS, MOS_INJECTED_MODULE_NAME, INFO), BOOST_PP_EMPTY())
