...
namespace fc {
  class variant;
  void to_variant(const mos::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, mos::chain::name& check);
} // fc
...
namespace mos::chain {
   ...
   private:
      ...
      friend void fc::from_variant(const fc::variant& v, mos::chain::name& check);
...
#define N(X) mos::chain::string_to_name(#X)

} // mos::chain

namespace std {
   template<> struct hash<mos::chain::name> : private hash<uint64_t> {
      typedef mos::chain::name argument_type;
...
FC_REFLECT( mos::chain::name, (value) )
