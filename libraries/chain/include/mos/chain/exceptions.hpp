...
#define MOS_ASSERT( expr, exc_type, FORMAT, ... )                \
   FC_MULTILINE_MACRO_BEGIN                                           \
   if( !(expr) )                                                      \
      FC_THROW_EXCEPTION( exc_type, FORMAT, __VA_ARGS__ );            \
   FC_MULTILINE_MACRO_END

#define MOS_THROW( exc_type, FORMAT, ... ) \
    throw exc_type( FC_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) );

/**
 * Macro inspired from FC_RETHROW_EXCEPTIONS
 * The main difference here is that if the exception caught isn't of type "mos::chain::chain_exception"
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define MOS_RETHROW_EXCEPTIONS(exception_type, FORMAT, ... ) \
   catch( const std::bad_alloc& ) {\
      throw;\
   } catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
   } catch (mos::chain::chain_exception& e) { \
 ...
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define MOS_CAPTURE_AND_RETHROW( exception_type, ... ) \
   catch( const std::bad_alloc& ) {\
      throw;\
   } catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
...
#define MOS_RECODE_EXC( cause_type, effect_type ) \
