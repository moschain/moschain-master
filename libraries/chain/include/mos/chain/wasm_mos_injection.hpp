...
#include <mos/chain/wasm_mos_binary_ops.hpp>
...
namespace mos { namespace chain { namespace wasm_injections {
   using namespace IR;
   ...
   struct injector_utils {
      ...
      static void add_import(Module& module, const char* func_name, int32_t& index ) {
         if (module.functions.imports.size() == 0 || registered_injected.find(func_name) == registered_injected.end() ) {
            ...
            decltype(module.functions.imports) new_import = { {{func_type_index}, MOS_INJECTED_MODULE_NAME, std::move(func_name)} };
...
#undef CLEAR
   ...
   struct call_depth_check_and_insert_checktime {
      ...
      static void accept( wasm_ops::instr* inst, wasm_ops::visitor_arg& arg ) {
         if ( global_idx == -1 ) {
            arg.module->globals.defs.push_back({{ValueType::i32, true}, {(I32) mos::chain::wasm_constraints::maximum_call_depth}});
         }
...
#undef INSERT_INJECTED
   ...
   constexpr const char* inject_which_op( uint16_t opcode ) {
      switch ( opcode ) {
         ...
         default:
            FC_THROW_EXCEPTION( mos::chain::wasm_execution_error, "Error, unknown opcode in injection ${op}", ("op", opcode));
      }
   }
      ...
      public:
         ...
         void inject() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::MOS_OperatorDecoderStream<std::conditional_t<full_injection, pre_op_full_injectors, pre_op_injectors>> pre_decoder(fd.code);
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::MOS_OperatorDecoderStream<std::conditional_t<full_injection, post_op_full_injectors, post_op_injectors>> post_decoder(fd.code);
...
}}} // namespace wasm_constraints, chain, mos
