...
namespace wasm_constraints = mos::chain::wasm_constraints;
...
class mos_vm_instantiated_module : public wasm_instantiated_module_interface {
      using backend_t = backend<apply_context, Impl>;
   public:
      
      mos_vm_instantiated_module(mos_vm_runtime<Impl>* runtime, std::unique_ptr<backend_t> mod) :
         _runtime(runtime),
      ...
      void apply(apply_context& context) override {
         ...
         } catch(mos::vm::timeout_exception&) {
            context.trx_context.checktime();
         } catch(mos::vm::wasm_memory_exception& e) {
            FC_THROW_EXCEPTION(wasm_execution_error, "access violation");
         } catch(mos::vm::exception& e) {
   ...
   private:
      mos_vm_runtime<Impl>*            _runtime;
      std::unique_ptr<backend_t> _instantiated_module;
};

template<typename Impl>
mos_vm_runtime<Impl>::mos_vm_runtime() {}

template<typename Impl>
void mos_vm_runtime<Impl>::immediately_exit_currently_running_module() {
   throw wasm_exit{};
}

template<typename Impl>
bool mos_vm_runtime<Impl>::inject_module(IR::Module& module) {
   return false;
}

template<typename Impl>
std::unique_ptr<wasm_instantiated_module_interface> mos_vm_runtime<Impl>::instantiate_module(const char* code_bytes, size_t code_size, std::vector<uint8_t>,
   ...
   try {
      ...
      return std::make_unique<mos_vm_instantiated_module<Impl>>(this, std::move(bkend));
   } catch(mos::vm::exception& e) {
      FC_THROW_EXCEPTION(wasm_execution_error, "Error building mos-vm interp: ${e}", ("e", e.what()));
   }
}

template class mos_vm_runtime<mos::vm::interpreter>;
template class mos_vm_runtime<mos::vm::jit>;
