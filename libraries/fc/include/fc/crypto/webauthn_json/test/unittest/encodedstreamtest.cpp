...
protected:
    ...
    void TestEncodedOutputStream(const char* expectedFilename, bool putBOM) {
        // Test FileWriteStream
        {
            ...
            EncodedOutputStream<FileEncoding, FileWriteStream> mos(os, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, mos);
                EXPECT_TRUE(success);
            }
            mos.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            EncodedOutputStream<FileEncoding, MemoryBuffer> mos(mb, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, MemoryEncoding>::Transcode(s, mos);
                EXPECT_TRUE(success);
    ...
    void TestAutoUTFOutputStream(UTFType type, bool putBOM, const char *expectedFilename) {
        // Test FileWriteStream
        {
            ...
            char buffer[16];
            FileWriteStream os(fp, buffer, sizeof(buffer));
            AutoUTFOutputStream<unsigned, FileWriteStream> mos(os, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, mos);
                EXPECT_TRUE(success);
            }
            mos.Flush();
            fclose(fp);
        ...
        {
            MemoryBuffer mb;
            AutoUTFOutputStream<unsigned, MemoryBuffer> mos(mb, type, putBOM);
            StringStream s(json_);
            while (s.Peek() != '\0') {
                bool success = Transcoder<UTF8<>, AutoUTF<unsigned> >::Transcode(s, mos);
                EXPECT_TRUE(success);
