...
int main(int, char*[]) {
    ...
    OutputStream mos(os, eis.GetType(), eis.HasBOM());                      // NEW
    PrettyWriter<OutputStream, UTF8<>, AutoUTF<unsigned> > writer(mos);     // CHANGED
#else
    ...
    OutputStream mos(os, true);                                             // NEW
    PrettyWriter<OutputStream, UTF8<>, UTF16LE<> > writer(mos);             // CHANGED
#endif
