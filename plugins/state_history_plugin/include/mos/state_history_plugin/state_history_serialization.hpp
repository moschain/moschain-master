...
#include <mos/chain/permission_link_object.hpp>
#include <mos/chain/permission_object.hpp>
#include <mos/chain/protocol_state_object.hpp>
#include <mos/chain/resource_limits.hpp>
#include <mos/chain/resource_limits_private.hpp>
#include <mos/chain/trace.hpp>
#include <mos/chain_plugin/chain_plugin.hpp>
#include <mos/state_history_plugin/state_history_plugin.hpp>
...
datastream<ST>& history_serialize_container(datastream<ST>& ds, const chainbase::database& db,
   ...
   fc::raw::pack(ds, unsigned_int(v.size()));
   for (auto& x : v) {
      MOS_ASSERT(!!x, mos::chain::plugin_exception, "null inside container");
      ds << make_history_serial_wrapper(db, *x);
   }
   return ds;
}
...
template <typename ST>
void history_pack_big_bytes(datastream<ST>& ds, const mos::chain::bytes& v) {
...
template <typename ST>
void history_pack_big_bytes(datastream<ST>& ds, const fc::optional<mos::chain::bytes>& v) {
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::account_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   fc::raw::pack(ds, as_type<mos::chain::block_timestamp_type>(obj.obj.creation_date));
   fc::raw::pack(ds, as_type<mos::chain::shared_string>(obj.obj.abi));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                      ds,
                           const history_serial_wrapper<mos::chain::account_metadata_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   fc::raw::pack(ds, as_type<bool>(obj.obj.is_privileged()));
   fc::raw::pack(ds, as_type<fc::time_point>(obj.obj.last_code_update));
   bool has_code = obj.obj.code_hash != mos::chain::digest_type();
   fc::raw::pack(ds, has_code);
   if (has_code) {
      fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_type));
      fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_version));
      fc::raw::pack(ds, as_type<mos::chain::digest_type>(obj.obj.code_hash));
   }
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::code_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_type));
   fc::raw::pack(ds, as_type<uint8_t>(obj.obj.vm_version));
   fc::raw::pack(ds, as_type<mos::chain::digest_type>(obj.obj.code_hash));
   fc::raw::pack(ds, as_type<mos::chain::shared_string>(obj.obj.code));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::table_id_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
operator<<(datastream<ST>&                                                                               ds,
           const history_context_wrapper<mos::chain::table_id_object, mos::chain::key_value_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   ...
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.payer.to_uint64_t()));
   fc::raw::pack(ds, as_type<mos::chain::shared_string>(obj.obj.value));
   return ds;
}
...
template <typename ST>
void serialize_secondary_index_data(datastream<ST>& ds, const mos::chain::key256_t& obj) {
...
template <typename ST, typename T>
datastream<ST>& serialize_secondary_index(datastream<ST>& ds, const mos::chain::table_id_object& context,
                                          const T& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
datastream<ST>&
operator<<(datastream<ST>&                                                                             ds,
           const history_context_wrapper<mos::chain::table_id_object, mos::chain::index64_object>& obj) {
...
datastream<ST>&
operator<<(datastream<ST>&                                                                              ds,
           const history_context_wrapper<mos::chain::table_id_object, mos::chain::index128_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}
...
datastream<ST>&
operator<<(datastream<ST>&                                                                              ds,
           const history_context_wrapper<mos::chain::table_id_object, mos::chain::index256_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}
...
datastream<ST>&
operator<<(datastream<ST>&                                                                                  ds,
           const history_context_wrapper<mos::chain::table_id_object, mos::chain::index_double_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}
...
datastream<ST>& operator<<(
    datastream<ST>&                                                                                       ds,
    const history_context_wrapper<mos::chain::table_id_object, mos::chain::index_long_double_object>& obj) {
   return serialize_secondary_index(ds, obj.context, obj.obj);
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds,
                           const history_serial_wrapper<mos::chain::shared_block_signing_authority_v0>& obj) {
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.threshold));
   history_serialize_container(ds, obj.db,
                               as_type<mos::chain::shared_vector<mos::chain::shared_key_weight>>(obj.obj.keys));

}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::shared_producer_authority>& obj) {
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.producer_name.to_uint64_t()));
   fc::raw::pack(ds, as_type<mos::chain::shared_block_signing_authority>(obj.obj.authority));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                            ds,
                           const history_serial_wrapper<mos::chain::shared_producer_authority_schedule>& obj) {
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.version));
   history_serialize_container(ds, obj.db,
                               as_type<mos::chain::shared_vector<mos::chain::shared_producer_authority>>(obj.obj.producers));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::chain_config>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                     ds,
                           const history_serial_wrapper<mos::chain::global_property_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(1));
   fc::raw::pack(ds, as_type<optional<mos::chain::block_num_type>>(obj.obj.proposed_schedule_block_num));
   fc::raw::pack(ds, make_history_serial_wrapper(
                         obj.db, as_type<mos::chain::shared_producer_authority_schedule>(obj.obj.proposed_schedule)));
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<mos::chain::chain_config>(obj.obj.configuration)));
   fc::raw::pack(ds, as_type<mos::chain::chain_id_type>(obj.obj.chain_id));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                           ds,
                           const history_serial_wrapper<mos::chain::generated_transaction_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.sender.to_uint64_t()));
   fc::raw::pack(ds, as_type<__uint128_t>(obj.obj.sender_id));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.payer.to_uint64_t()));
   fc::raw::pack(ds, as_type<mos::chain::transaction_id_type>(obj.obj.trx_id));
   fc::raw::pack(ds, as_type<mos::chain::shared_string>(obj.obj.packed_trx));
   return ds;
}
...
datastream<ST>&
operator<<(datastream<ST>&                                                                                ds,
           const history_serial_wrapper<mos::chain::protocol_state_object::activated_protocol_feature>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<mos::chain::digest_type>(obj.obj.feature_digest));
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.activation_block_num));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::protocol_state_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::shared_key_weight>& obj) {
   fc::raw::pack(ds, as_type<mos::chain::shared_public_key>(obj.obj.key));
   fc::raw::pack(ds, as_type<uint16_t>(obj.obj.weight));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::permission_level>& obj) {
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                      ds,
                           const history_serial_wrapper<mos::chain::permission_level_weight>& obj) {
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<mos::chain::permission_level>(obj.obj.permission)));
   fc::raw::pack(ds, as_type<uint16_t>(obj.obj.weight));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::wait_weight>& obj) {
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::shared_authority>& obj) {
   fc::raw::pack(ds, as_type<uint32_t>(obj.obj.threshold));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::permission_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.owner.to_uint64_t()));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   if (obj.obj.parent._id) {
      auto&       index  = obj.db.get_index<mos::chain::permission_index>();
      const auto* parent = index.find(obj.obj.parent);
      if (!parent) {
         auto& undo = index.stack().back();
         auto  it   = undo.removed_values.find(obj.obj.parent);
         MOS_ASSERT(it != undo.removed_values.end(), mos::chain::plugin_exception,
   ...
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<mos::chain::shared_authority>(obj.obj.auth)));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                     ds,
                           const history_serial_wrapper<mos::chain::permission_link_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                                      ds,
                           const history_serial_wrapper<mos::chain::resource_limits::resource_limits_object>& obj) {
   MOS_ASSERT(!obj.obj.pending, mos::chain::plugin_exception,
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                                 ds,
                           const history_serial_wrapper<mos::chain::resource_limits::usage_accumulator>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                                     ds,
                           const history_serial_wrapper<mos::chain::resource_limits::resource_usage_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.owner.to_uint64_t()));
   fc::raw::pack(ds, make_history_serial_wrapper(
                         obj.db, as_type<mos::chain::resource_limits::usage_accumulator>(obj.obj.net_usage)));
   fc::raw::pack(ds, make_history_serial_wrapper(
                         obj.db, as_type<mos::chain::resource_limits::usage_accumulator>(obj.obj.cpu_usage)));
...
datastream<ST>&
operator<<(datastream<ST>&                                                                            ds,
           const history_serial_wrapper<mos::chain::resource_limits::resource_limits_state_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<mos::chain::resource_limits::usage_accumulator>(
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                     ds,
                           const history_serial_wrapper<mos::chain::resource_limits::ratio>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                                        ds,
                           const history_serial_wrapper<mos::chain::resource_limits::elastic_limit_parameters>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   ...
   fc::raw::pack(
       ds, make_history_serial_wrapper(obj.db, as_type<mos::chain::resource_limits::ratio>(obj.obj.contract_rate)));
   fc::raw::pack(
       ds, make_history_serial_wrapper(obj.db, as_type<mos::chain::resource_limits::ratio>(obj.obj.expand_rate)));
   return ds;
}
...
datastream<ST>&
operator<<(datastream<ST>&                                                                             ds,
           const history_serial_wrapper<mos::chain::resource_limits::resource_limits_config_object>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(
       ds, make_history_serial_wrapper(
               obj.db, as_type<mos::chain::resource_limits::elastic_limit_parameters>(obj.obj.cpu_limit_parameters)));
   fc::raw::pack(
       ds, make_history_serial_wrapper(
               obj.db, as_type<mos::chain::resource_limits::elastic_limit_parameters>(obj.obj.net_limit_parameters)));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::action>& obj) {
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.account.to_uint64_t()));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.name.to_uint64_t()));
   history_serialize_container(ds, obj.db, as_type<std::vector<mos::chain::permission_level>>(obj.obj.authorization));
   fc::raw::pack(ds, as_type<mos::bytes>(obj.obj.data));
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::action_receipt>& obj) {
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.receiver.to_uint64_t()));
   fc::raw::pack(ds, as_type<mos::chain::digest_type>(obj.obj.act_digest));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.global_sequence));
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.recv_sequence));
   history_serialize_container(ds, obj.db, as_type<flat_map<mos::name, uint64_t>>(obj.obj.auth_sequence));
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_serial_wrapper<mos::chain::account_delta>& obj) {
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.account.to_uint64_t()));
...
inline fc::optional<uint64_t> cap_error_code( const fc::optional<uint64_t>& error_code ) {
   ...
   const uint64_t upper_limit = static_cast<uint64_t>(mos::chain::system_error_code::generic_system_error);
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const history_context_wrapper<bool, mos::chain::action_trace>& obj) {
   ...
   if (obj.obj.receipt) {
      fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<mos::chain::action_receipt>(*obj.obj.receipt)));
   }
   fc::raw::pack(ds, as_type<uint64_t>(obj.obj.receiver.to_uint64_t()));
   fc::raw::pack(ds, make_history_serial_wrapper(obj.db, as_type<mos::chain::action>(obj.obj.act)));
   ...
   history_serialize_container(ds, obj.db, as_type<flat_set<mos::chain::account_delta>>(obj.obj.account_ram_deltas));
...
datastream<ST>& operator<<(datastream<ST>&                                                    ds,
                           const history_context_wrapper<std::pair<uint8_t, bool>,
                                                         mos::augmented_transaction_trace>& obj) {
   auto& trace = *obj.obj.trace;
   bool  debug_mode = obj.context.second;
   fc::raw::pack(ds, fc::unsigned_int(0));
   fc::raw::pack(ds, as_type<mos::chain::transaction_id_type>(trace.id));
   if (trace.receipt) {
      if (trace.failed_dtrx_trace && trace.receipt->status.value == mos::chain::transaction_receipt_header::soft_fail)
         fc::raw::pack(ds, uint8_t(mos::chain::transaction_receipt_header::executed));
      else
   ...
   history_context_serialize_container(ds, obj.db, debug_mode,
                                       as_type<std::vector<mos::chain::action_trace>>(trace.action_traces));

   fc::raw::pack(ds, bool(trace.account_ram_delta));
   if (trace.account_ram_delta) {
      fc::raw::pack(
          ds, make_history_serial_wrapper(obj.db, as_type<mos::chain::account_delta>(*trace.account_ram_delta)));
   }
   ...
   if (trace.failed_dtrx_trace) {
      uint8_t stat = mos::chain::transaction_receipt_header::hard_fail;
      if (trace.receipt && trace.receipt->status.value == mos::chain::transaction_receipt_header::soft_fail)
         stat = mos::chain::transaction_receipt_header::soft_fail;
      std::pair<uint8_t, bool> context = std::make_pair(stat, debug_mode);
      fc::raw::pack( //
          ds, make_history_context_wrapper(
                  obj.db, context, mos::augmented_transaction_trace{trace.failed_dtrx_trace, obj.obj.partial}));
   }
   ...
   if (include_partial) {
      ...
      fc::raw::pack(ds, fc::unsigned_int(0));
      fc::raw::pack(ds, as_type<mos::chain::time_point_sec>(partial.expiration));
      ...
      fc::raw::pack(ds, as_type<mos::chain::extensions_type>(partial.transaction_extensions));
      fc::raw::pack(ds, as_type<std::vector<mos::chain::signature_type>>(partial.signatures));
      fc::raw::pack(ds, as_type<std::vector<mos::bytes>>(partial.context_free_data));
   }
...
template <typename ST>
datastream<ST>& operator<<(datastream<ST>&                                                           ds,
                           const history_context_wrapper<bool, mos::augmented_transaction_trace>&  obj) {
   std::pair<uint8_t, bool> context = std::make_pair(mos::chain::transaction_receipt_header::hard_fail, obj.context);
   ds << make_history_context_wrapper(obj.db, context, obj.obj);
   return ds;
}

template <typename ST>
datastream<ST>& operator<<(datastream<ST>& ds, const mos::get_blocks_result_v0& obj) {
