#!/bin/bash
#
# mos-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# mos-tn_bounce.sh and mos-tn_roll.sh scripts.

connected="0"

rundir=programs/nodemos
prog=nodemos

# Quote any args that are "*", so they are not expanded
qargs=`echo "$*" | sed -e 's/ \* / "*" /' -e 's/ \*$/ "*"/'`

if [ "$PWD" != "$MOS_HOME" ]; then
    echo $0 must only be run from $MOS_HOME
    exit -1
fi
...
datadir=var/lib/node_$MOS_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/mos/node_$MOS_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/mos/node_$MOS_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
