...
function usage() {
  ...
  -i DIR      Directory where mos is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the mos data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export MOS_INSTALL_DIR=${INSTALL_LOCATION:-$MOS_INSTALL_DIR}

if [[ ! -d "${MOS_INSTALL_DIR}" ]]; then
   echo "[MOS installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${MOS_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/mos/mos/issues/6940
   echo "[MOS installation found: ${MOS_INSTALL_DIR}]" && INSTALL_PATHS+=("${MOS_INSTALL_DIR}") # MOS_INSTALL_DIR set in .environment
   while true; do
      [[ $NONINTERACTIVE == false ]] && read -p "Do you wish to remove the installation in ${MOS_INSTALL_DIR}? (y/n) " PROCEED
      case $PROCEED in
         "" ) echo "What would you like to do?";;
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/mos")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/mos")
            fi
            # Version < 1.8.0; Before we started using ~/mos/1.8.x
            ...
            echo " - MOS Removal Complete"
            break;;
         1 | false | [Nn]* ) echo " - Cancelled MOS Removal!"; exit 1;;
         * ) echo "Please type 'y' for yes or 'n' for no.";;
      esac
   done
fi

echo "[Removing MOS Dependencies]"
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/mos_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
